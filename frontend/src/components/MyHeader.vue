...
async function submit() {
  ...
  if (loggedIn.value) {
    // Logout
    try {
      const response = await axios.post('http://yowayoli.com/api/logout');
    ...
    // Login
    try {
      const response = await axios.post('http://yowayoli.com/api/login', {
        email: email.value,
        psw: password.value
      });
      if (response.data.success) {
    ...
    // Registrierung
    try {
      await axios.post('http://yowayoli.com/api/create-user', {
